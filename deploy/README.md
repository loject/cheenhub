...
Скрипт создаст `.env.production` с паролем Postgres и JWT-ключами. Проверь значения после генерации; секреты не коммитятся. Для образов из GitHub Container Registry укажи:

```dotenv
CHEENHUB_BACKEND_IMAGE=ghcr.io/<owner>/<repo>/backend
CHEENHUB_WEB_IMAGE=ghcr.io/<owner>/<repo>/web
CHEENHUB_IMAGE_TAG=v1.0.0
```
...
После релиза обнови `CHEENHUB_IMAGE_TAG` на сервере и перезапусти compose. Если GHCR package приватный, сначала выполни на сервере `docker login ghcr.io`.
