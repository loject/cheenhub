...
pub(crate) async fn enumerate_audio_input_devices() -> AudioInputDevicesResult {
    ...
    let devices = match tokio::task::spawn_blocking(enumerate_blocking).await {
        ...
        Ok(Err(error)) => {
            warn!(%error, "PulseAudio input device enumeration failed");
            None
        }
        Err(error) => {
            warn!(%error, "PulseAudio input enumeration worker failed");
            None
        }
    };
    debug!(elapsed_ms = %started.elapsed().as_millis(), device_count = devices.as_ref().map_or(0, Vec::len), available = devices.is_some(), "finished PulseAudio input enumeration");
...
fn enumerate_blocking() -> Result<Vec<AudioInputDevice>, String> {
    let deadline = Instant::now() + Duration::from_secs(3);
    let mut mainloop = Mainloop::new().ok_or("cannot create PulseAudio mainloop")?;
    let mut context = Context::new(&mainloop, "CheenHub input devices")
        ...
        .get_source_info_list(move |result| match result {
            ListResult::Item(info) => {
                if let Some(device) = input_device(
                    ...
                    info.monitor_of_sink,
                ) {
                    callback_entries.borrow_mut().push(device);
                }
            }
...
fn disambiguate(mut devices: Vec<AudioInputDevice>) -> Vec<AudioInputDevice> {
...
/// На Linux отдельное разрешение не требуется; повторяет перечисление устройств.
