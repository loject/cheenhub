...
pub(crate) fn VoiceControls(target: VoiceRoomTarget) -> Element {
    ...
    let leave_label = if target.kind == VoiceRoomTargetKind::DirectMessage {
        "Завершить звонок"
    } else {
    ...
    rsx! {
        div { class: "voice-floating-controls pointer-events-none absolute inset-x-0 bottom-0 z-50 flex justify-center p-5",
            div { class: "voice-controls-panel pointer-events-auto flex items-center gap-3 rounded-[24px] border border-zinc-800 bg-zinc-950/85 p-2 shadow-[0_18px_50px_rgba(0,0,0,0.38)] drop-shadow-[0_12px_24px_rgba(0,0,0,.28)] backdrop-blur-xl",
                button {
                    r#type: "button",
                    disabled: !media_controls_enabled || microphone_starting,
                    class: if microphone_speaking {
                        "voice-control-button group relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-xl border border-emerald-300/80 bg-emerald-500/20 text-emerald-50 shadow-[0_0_0_1px_rgba(52,211,153,.25),0_14px_36px_rgba(16,185,129,.22)] transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-500/25 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60"
                    } else if microphone_live {
                        "voice-control-button group relative flex h-14 w-14 items-center justify-center rounded-xl border border-emerald-500/35 bg-emerald-500/15 text-emerald-100 transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-emerald-400/45 hover:bg-emerald-500/20 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60"
                    } else {
                    ...
                    span { class: control_icon_class(microphone_live),
                ...
                button {
                    ...
                    class: if camera_live {
                        "voice-control-button group relative flex h-14 w-14 items-center justify-center rounded-xl border border-cyan-400/45 bg-cyan-500/15 text-cyan-100 shadow-[0_0_0_1px_rgba(34,211,238,.16),0_14px_36px_rgba(6,182,212,.14)] transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-cyan-300/55 hover:bg-cyan-500/20 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60"
                    } else if matches!(camera_status, CameraStatus::PermissionDenied | CameraStatus::Error(_)) {
                        "voice-control-button group relative flex h-14 w-14 items-center justify-center rounded-xl border border-amber-500/35 bg-amber-500/10 text-amber-100 transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-amber-400/45 hover:bg-amber-500/15 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60"
                    } else {
                        "voice-control-button group relative flex h-14 w-14 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900/80 text-zinc-200 transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-zinc-700 hover:bg-zinc-900 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60"
                    },
                    ...
                    span { class: "pointer-events-none absolute bottom-[calc(100%+10px)] left-1/2 -translate-x-1/2 translate-y-1 whitespace-nowrap rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-1.5 text-[12px] font-medium text-zinc-200 opacity-0 transition-[opacity,transform] duration-150 group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100", "{camera_label}" }
                    span { class: control_icon_class(camera_live),
                        svg { class: "h-5 w-5", fill: "none", stroke: "currentColor", stroke_width: "1.9", view_box: "0 0 24 24", "aria-hidden": "true",
                ...
                button {
                    ...
                    class: if screen_share_live {
                        "voice-control-button group relative flex h-14 w-14 items-center justify-center rounded-xl border border-sky-400/45 bg-sky-500/15 text-sky-100 shadow-[0_0_0_1px_rgba(56,189,248,.16),0_14px_36px_rgba(14,165,233,.14)] transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-sky-300/55 hover:bg-sky-500/20 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60"
                    ...
                    span { class: control_icon_class(!screen_share_live),
                        svg { class: "h-5 w-5", fill: "none", stroke: "currentColor", stroke_width: "1.9", view_box: "0 0 24 24", "aria-hidden": "true",
                ...
                button {
                    r#type: "button",
                    disabled: is_leaving,
                    class: "voice-control-button group relative flex h-14 w-14 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-900/80 text-zinc-300 transition-[transform,background-color,border-color,color,box-shadow,opacity] duration-[180ms] hover:-translate-y-0.5 hover:border-red-500/30 hover:bg-red-500/10 hover:text-red-200 active:scale-[0.96] disabled:cursor-wait disabled:opacity-60",
...
fn control_icon_class(visible: bool) -> &'static str {
    if visible {
        "pointer-events-none absolute z-10 grid scale-100 place-items-center opacity-100 blur-0 transition-[transform,opacity,filter] duration-300 ease-[cubic-bezier(0.2,0,0,1)]"
    } else {
