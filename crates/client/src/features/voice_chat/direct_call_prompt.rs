...
pub(super) fn DirectCallPrompt(call: DirectCallSnapshot, exiting: bool) -> Element {
    ...
    rsx! {
        div {
            ...
            div {
                class: if exiting {
                    "direct-call-prompt-card direct-call-prompt-card-exit w-full max-w-sm rounded-[28px] bg-zinc-950/95 p-3 shadow-[0_0_0_1px_rgba(255,255,255,.10),0_28px_90px_rgba(0,0,0,.58)]"
                } else {
                    "direct-call-prompt-card w-full max-w-sm rounded-[28px] bg-zinc-950/95 p-3 shadow-[0_0_0_1px_rgba(255,255,255,.10),0_28px_90px_rgba(0,0,0,.58)]"
                },
                div {
                    class: "rounded-[20px] bg-zinc-900/80 px-5 py-6 text-center",
                    p {
                        class: "direct-call-enter-segment text-[12px] font-medium uppercase tracking-[0.16em] text-emerald-300",
                        "Входящий звонок"
                    }
                    UserAvatar {
                    ...
                    div { class: "direct-call-enter-segment direct-call-enter-segment-actions mt-6 grid grid-cols-2 gap-3",
                        ...
                        button {
                            r#type: "button",
                            disabled: busy,
                            class: "flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 text-[13px] font-semibold text-emerald-950 shadow-[0_12px_32px_rgba(16,185,129,.24)] transition-[scale,background-color,opacity] duration-150 ease-out active:scale-[0.96] disabled:cursor-wait disabled:opacity-60",
